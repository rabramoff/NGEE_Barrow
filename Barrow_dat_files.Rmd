...
```{r}
  #headers for control board     
   colnames(data) <- c("start", "date", "time", "id", "log", "BoardTemp", "U.R.8.5", "U.R.8.10", "U.R.8.15", "U.R.8.20", "U.R.8.25", "H.R.8.5", "H.R.8.10", "H.R.8.15", "H.R.8.20", "H.R.8.25", 
                               "H.M.6.5", "H.M.6.10", "H.M.6.20", "H.M.6.35", "H.M.6.50", "NotKnown1", "UAvg", "HAvg", "TempDiff", "HeaterSetting", "NotKnown2", "Voltage", "HeaterIntervalCount", "NotKnown3", "end")
  #headers for monitoring board 
       colnames(data2) <- c("start", "date", "time", "id", "log", "BoardTemp", "H.M.10.5", "H.M.10.10", "H.M.10.20", "H.M.10.35", "H.M.10.50", "U.M.6.5", "U.M.6.10", "U.M.6.20", "U.M.6.35", "U.M.6.50", 
                               "U.M.10.5", "U.M.10.10", "U.M.10.20", "U.M.10.35", "U.M.10.50", "NotKnown1",
                               "UAvg", "HAvg", "TempDiff", "HeaterSetting", "NotKnown2", "Voltage", "HeaterIntervalCount", "NotKnown3", "end")
```
...
```{r}
  ...
  #write.csv(wide2, file = "/Users/rzabramoff/Desktop/mywidefile.csv")
...
```{r}
  ...
  plot(wide2$time, wide2$Uavg, cex = 0.5, ylab = expression("Temperature (" ~ degree*C ~ ")"), xlab = "Time", xaxt = "n")
  r <- range(wide2$time)
  axis.POSIXct(1, at = seq(r[1], r[2], by = "days"), format = "%m/%d - %I%p")
  points(wide2$time, wide2$Havg, cex = 0.5, col = 2)
  points(wide2$time, wide2$TempDiff, cex = 0.5, col = 4)
  legend("topright",c("Uavg","Havg", "TempDiff"),
         pch = c(1,1,1),
         col=c(1,2,4),cex=1.3
  )
