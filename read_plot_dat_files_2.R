...
barrowData <- function(path, path2) {
  ...
  ch1 <- lapply(firstFolder, whitespace)
  ch2 <- lapply(secondFolder, whitespace)
  ...
  newdata1 <- lapply(ch1, parsefx)
  newdata2 <- lapply(ch2, parsefx)
  
  #enter column headers below
  newdata1 <- as.data.frame(newdata1)
  newdata2 <- as.data.frame(newdata2)
  colnames(newdata1) <- c("start", "date", "time", "id", "plot", "H.1.1", "H.1.2", "H.1.3", "H.1.4", "H.1.5", "H.2.1", 
  ...
  colnames(newdata2) <- c("start", "date", "time", "id", "plot", "M.1.1", "M.1.2", "M.1.3", "M.1.4", "M.1.5", "M.2.1", 
  ...
  long1 <- meltLong(wide1)
  long2 <- meltLong(wide2)
  ...
  plot(long$time[1:1000], long$value[1:1000])
}
